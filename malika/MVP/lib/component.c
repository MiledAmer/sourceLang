...
void imported_components_ts(DynamicBuffer* import_buffer) {
    buffer_append(import_buffer, "// Importations générées automatiquement :\n");

    for (int i = 0; i < imported_count; i++) {
        const char* name = imported_components[i].name;
        const char* path = imported_components[i].Path;

        // Ajouter chaque import à la suite du buffer
        char line[256];
        snprintf(line, sizeof(line), "import %s from %s;\n", name, path);
        buffer_append(import_buffer, line);
    }

    buffer_append(import_buffer, "\n");
}
