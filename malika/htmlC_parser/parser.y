...
%token <strval> IDENTIFIER RETURN DOUBLE_QUOTE EQUALS TYPE SEMICOLON FROM IMPORT 
%type <strval> element parameters typed_param_list typed_param function html_content_list html_content html_balise_open html_balise_close html_inner html_balise_autoferme attributes attribute
%start program
...
function:
    /* empty */ { $$ = strdup(""); }
    |  RETURN LPAREN html_content_list RPAREN SEMICOLON
    { 
...
html_content_list:
    html_content
    { 
        char* tmp = malloc(strlen($1) + 1); // Allouer mémoire pour le contenu HTML
        sprintf(tmp, "%s", $1);
        free($1);
        $$ = tmp;
    }
    | html_content_list html_content
    { 
        char* tmp = malloc(strlen($1) + strlen($2) + 1); // Allouer mémoire pour le contenu HTML
        sprintf(tmp, "%s %s", $1, $2);
        free($1);
        free($2);
        $$ = tmp;
...
html_content:
    html_balise_open html_content_list html_balise_close
    { 
